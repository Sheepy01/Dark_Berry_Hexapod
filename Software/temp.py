...
FEMUR_CAL = [-40+F_CAL_VAL, -35+F_CAL_VAL, -40+F_CAL_VAL, -18+F_CAL_VAL,  -25+F_CAL_VAL,  -15+F_CAL_VAL]
TIBIA_CAL = [20+T_CAL_VAL, 25+T_CAL_VAL, 25+T_CAL_VAL, 25+T_CAL_VAL, 30+T_CAL_VAL, 20+T_CAL_VAL]
...
def map_input(input_value, input_min, input_max, output_min, output_max):
    ...
    normalized_input = (input_value - input_min) / input_range
    mapped_value = normalized_input * output_range + output_min
    return mapped_value
...
def process_gamepad():
    ...
    for event in pygame.event.get():
        ...
        if event.type == pygame.JOYAXISMOTION:
            ...
            if event.axis == R_STICK_Y_AXIS:
                R3_y = event.value
                commandedX = map_input(R3_y, right_joystick_y_min, right_joystick_y_max, 127, -127)
                ...
                cosRotY = cos((map_input(R3_y, 0, 255, A12DEG, -A12DEG))/1000000.0)
                #temp = map_input(L3_y, left_joystick_y_min, left_joystick_y_max, 0, 255)
            
            # Right Joystick X Axis
            if event.axis == R_STICK_X_AXIS:
                R3_x = event.value
                commandedY = map_input(R3_x, right_joystick_x_min, right_joystick_x_max, -127, 127)
                ...
                cosRotX = cos((map_input(R3_x, 0, 255, A12DEG, -A12DEG))/1000000.0)
                # temp = map_input(L3_x, left_joystick_x_min, left_joystick_x_max, 0, 255)

            # Right Joystick X Axis
            if event.axis == L_STICK_X_AXIS:
                L3_x = event.value
                commandedR = map_input(L3_x, left_joystick_x_min, left_joystick_x_max, -127, 127)
                sinRotZ = sin((map_input(L3_x, 0, 255, -A30DEG, A30DEG))/1000000.0)
                cosRotZ = cos((map_input(L3_x, 0, 255, -A30DEG, A30DEG))/1000000.0)
            ...
            # Left Joystick Y Axis
                ...
                if translateZ > 127.0:
                    translateZ = map_input(translateZ, 128.0, 255.0, 0, 1.5*TRAVEL)
                else:
                    translateZ = map_input(translateZ, 0.0, 127.0, -1.5*TRAVEL , 0)
                # temp = map_input(R3_y, left_joystick_y_min, right_joystick_y_max, 0, 255)
...
# Function to calculate the position using a sine ease-in-out function
def sine_ease_in_out(t, min_val, max_val, total_time):
    # Calculate the position using a sine ease-in-out function
    position = min_val + (max_val - min_val) * 0.5 * (1.0 - math.cos(t / total_time * math.pi))
    return position
    ...
    if (L3 < (TIBIA_LENGTH + FEMUR_LENGTH)) and (L3 > (TIBIA_LENGTH - FEMUR_LENGTH)):
        ...
        if leg_number == 0:
            if leg1_IK_control:  # flag for IK or manual control of leg
                ...
                kit1.servo[COXA1_SERVO].angle = int(theta_coxa)
                kit1.servo[FEMUR1_SERVO].angle = int(theta_femur)
                kit1.servo[TIBIA1_SERVO].angle = int(theta_tibia)

        if leg_number == 1:
            theta_coxa += 90.0  # compensate for leg mounting
            theta_coxa = max(min(theta_coxa, 180.0), 0.0)
            #print(theta_coxa, theta_femur, theta_tibia)
            kit1.servo[COXA2_SERVO].angle = int(theta_coxa)
            kit1.servo[FEMUR2_SERVO].angle = int(theta_femur)
            kit1.servo[TIBIA2_SERVO].angle = int(theta_tibia)
    
        if leg_number == 2:
            theta_coxa += 135.0  # compensate for leg mounting
            theta_coxa = max(min(theta_coxa, 180.0), 0.0)
            #print(theta_coxa, theta_femur, theta_tibia)
            kit1.servo[COXA3_SERVO].angle = int(theta_coxa)
            kit1.servo[FEMUR3_SERVO].angle = int(theta_femur)
            kit1.servo[TIBIA3_SERVO].angle = int(theta_tibia)
        ...
        if leg_number == 3:
            ...
            theta_coxa = max(min(theta_coxa, 180.0), 0.0)
            print(theta_coxa, theta_femur, theta_tibia)
            kit2.servo[COXA4_SERVO].angle = int(theta_coxa)
            kit2.servo[FEMUR4_SERVO].angle = int(theta_femur)
            kit2.servo[TIBIA4_SERVO].angle = int(theta_tibia)
        ...
        if leg_number == 4:
            ...
            theta_coxa = max(min(theta_coxa, 180.0), 0.0)
            #print(theta_coxa, theta_femur, theta_tibia)
            kit2.servo[COXA5_SERVO].angle = int(theta_coxa)
            kit2.servo[FEMUR5_SERVO].angle = int(theta_femur)
            kit2.servo[TIBIA5_SERVO].angle = int(theta_tibia)
        ...
        if leg_number == 5:
            if leg6_IK_control:  # flag for IK or manual control of leg
                ...
                theta_coxa = max(min(theta_coxa, 180.0), 0.0)
                #print(theta_coxa, theta_femur, theta_tibia)
                kit2.servo[COXA6_SERVO].angle = int(theta_coxa)
                kit2.servo[FEMUR6_SERVO].angle = int(theta_femur)
                kit2.servo[TIBIA6_SERVO].angle = int(theta_tibia)
